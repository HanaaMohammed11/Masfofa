...
export default function AdminUserCard({ user, index, onEmpClick }) {
  ...
  return (
    <tr
      className={`${rowColor} cursor-pointer  font-semibold`}
      onClick={() => {
        // handleCardClick();
        onEmpClick(user);
      }}
    >
      <td className="px-6 py-4 w-44">
        <div className="flex items-center">
          ...
          <span className="mr-4 truncate font-semibold text-xl ml-4">
            {user.employeeName}
          </span>
        </div>
      </td>
...
const profileContainerStyle = {
  ...
  background:
    "linear-gradient(white, white) padding-box, linear-gradient(45deg, #000000 40%, #404040 60%, #C0C0C0 100%) border-box",
};
